...
IMPORTANT — `pages/contact.html` n'a volontairement PAS de footer (choix de Bastien : pas nécessaire sur cette page, laisse plus de place à l'image de fond). Ne pas "corriger" ça en rajoutant `#footer-placeholder` — déjà fait une fois par erreur (13/07) et annulé.

Fix overscroll mobile (v2, 14/07) : le premier fix (fond de `<html>` seul) ne suffisait pas sur mobile réel (Safari iOS semble se baser sur le fond de `<body>` plutôt que `<html>` pour l'effet rebond, donc le flash de crème persistait — vu par Bastien sur son téléphone, capture à l'appui). Corrigé en mettant `html` ET `body` en vert foncé (couleur du footer) comme "couleur canvas" pour le rebond, et en donnant explicitement le fond crème à `<main>` (qui en dépendait implicitement de `body` jusque-là). Si le souci persiste encore sur mobile réel après ce fix, il faudra le vérifier avec un vrai appareil ou un émulateur mobile complet (le resize de fenêtre Chrome ne reproduit pas fidèlement le rebond de scroll tactile).
